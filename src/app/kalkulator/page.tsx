      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-6 sm:py-8">
        ...
        {tab === 'kalkulator' && (
          <div className="space-y-6">
            ...
            {!loading && error && (
              <div className="flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-200 text-red-800">
                ...
                <div className="flex-1">
                  ...
                  <button onClick={() => setError(null)} className="text-xs text-red-600 hover:text-red-800 underline font-semibold mt-2 block">
                    Coba Lagi
                  </button>
                </div>
              </div>
            ...
            {!loading && step === 1 && (
              <StepTirkah value={tirkah} onChange={setTirkah} onNext={() => setStep(2)} />
            )}
            {!loading && step === 2 && (
              <StepAhliWaris value={ahliWarisList} onChange={setAhliWarisList} onNext={handleHitung} onBack={() => setStep(1)} />
            )}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-arabic text-base text-emerald-800 font-bold">{preset.arab}</p>
                      <p className="text-xs text-slate-600 font-medium">Komposisi: {preset.desc}</p>
            ...
            <div className="card space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm sm:text-base">1. Furudh Muqaddarah (6 Pecahan Pasti)</h3>
                </div>
                <span className="text-arabic text-sm sm:text-base text-emerald-800 font-bold">الفروض المقدرة في كتاب الله</span>
              </div>
              <div className="overflow-x-auto">
                <table className="tbl">
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs sm:text-sm">
                    {[
                      ['1/2', 'النصف', 'Suami (tanpa anak/cucu); Anak Perempuan tunggal; Cucu Perempuan tunggal; Saudari Sekandung tunggal; Saudari Seayah tunggal.'],
                      ['1/4', 'الربع', 'Suami (ada anak/cucu); Istri (tanpa anak/cucu).'],
                      ['1/8', 'الثمن', 'Istri (ada anak/cucu).'],
                      ['2/3', 'الثلثان', '2+ Anak Perempuan; 2+ Cucu Perempuan; 2+ Saudari Sekandung; 2+ Saudari Seayah.'],
                      ['1/3', 'الثلث', 'Ibu (tanpa anak/cucu & < 2 saudara/i); 2+ Saudara/i Seibu (dibagi rata).'],
                      ['1/6', 'السدس', 'Ibu (ada anak/cucu atau 2+ saudara/i); Ayah (ada anak/cucu); Kakek; Nenek; dll.'],
            ...
            <div className="card space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm sm:text-base">2. Ashabah (Penerima Sisa Harta)</h3>
                </div>
                <span className="text-arabic text-sm sm:text-base text-blue-800 font-bold">أقسام العصبة في الإرث</span>
              </div>
              <div className="grid grid-cols-1 gap-3">
                {[
                  { badge: 'Ashabah Bin-Nafsih (عصبة بنفسه)', ar: '12 Urutan Derajat Laki-laki', desc: 'Anak Lk → Cucu Lk → Ayah → Kakek → Saudara Sekandung → Saudara Seayah → Keponakan Sekandung → Keponakan Seayah → Paman Sekandung → Paman Seayah → Sepupu Sekandung → Sepupu Seayah → Mu\'tiq.' },
                  { badge: 'Ashabah Bil-Ghair (عصبة بغيره)', ar: 'Ditarik Saudara Lk (Rasio 2:1)', desc: 'Anak Pr + Anak Lk; Cucu Pr + Cucu Lk; Saudari Sekandung + Saudara Sekandung; Saudari Seayah + Saudara Seayah. (للذكر مثل حظ الأنثيين).' },
                  { badge: "Ashabah Ma'al-Ghair (عصبة مع غيره)", ar: 'Bersama Anak/Cucu Perempuan', desc: 'Saudari Sekandung atau Saudari Seayah ketika hadir bersama Anak Perempuan atau Cucu Perempuan, mengambil seluruh sisa harta.' },
                ].map(item => (
                  <div key={item.badge} className="rounded-xl border border-blue-200 bg-blue-50/50 p-4">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="badge-blue text-xs font-bold">{item.badge}</span>
                      <span className="text-arabic text-sm text-blue-800 font-bold">{item.ar}</span>
                    </div>
            ...
            <div className="card space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm sm:text-base">3. Masalah Khusus (المسائل الخاصة)</h3>
                </div>
                <span className="text-arabic text-sm sm:text-base text-purple-800 font-bold">الغرواوين والمشتركة والأكدرية</span>
              </div>
              <div className="grid grid-cols-1 gap-3">
                {[
                  { nama: 'Al-Gharrawain (Al-Umariyyatain)', arab: 'المسألة الغراوية', pemicu: 'Suami/Istri + Ibu + Ayah', aturan: 'Ibu mendapat 1/3 dari SISA harta setelah bagian pasangan diambil, bukan 1/3 total.' },
                  { nama: 'Al-Musytarakah (Al-Himariyah)', arab: 'المسألة المشتركة', pemicu: 'Suami + Ibu + 2+ Saudara/i Seibu + Saudara Lk Sekandung', aturan: 'Saudara Lk Sekandung ikut berbagi rata dalam porsi 1/3 bersama Saudara/i Seibu.' },
                  { nama: 'Al-Akdariyyah', arab: 'المسألة الأكدرية', pemicu: 'Suami + Ibu + Kakek + Saudari Kandung', aturan: 'Porsi Kakek + Saudari digabung lalu dibagi 2:1. Diselesaikan dengan Tashih × 3 = 27.' },
                ].map(kk => (
                  <div key={kk.nama} className="rounded-xl border border-purple-200 bg-purple-50/50 p-4">
                    ...
                    <p className="text-xs text-slate-600 mb-1 leading-relaxed"><strong className="text-slate-800">Pemicu:</strong> {kk.pemicu}</p>
                    <p className="text-xs text-slate-700 leading-relaxed"><strong className="text-purple-800">Penyelesaian:</strong> {kk.aturan}</p>
                  </div>
                ))}
              </div>
            </div>
